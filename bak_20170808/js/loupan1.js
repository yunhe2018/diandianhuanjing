...
function deletePoint() {
    ...
    for (var i = 0; i < allOverlay.length - 1; i++) {
        if (allOverlay[i].Yc) {
            if (allOverlay[i].Yc.innerHTML.indexOf('lookArtHear') > -1) {
...
function deleteCircle() {
    ...
    for (var i = 0; i < allOverlay.length - 1; i++) {
        if (allOverlay[i].xa) {
            if (allOverlay[i].xa == g_const_Map_Distance) {
                map.removeOverlay(allOverlay[i]);
                return false;
            }
        }
...
var Baidu = {
    LoadData: function (point, projectname, address, pic, number,index,updatetime) {
        var html = "<div class=\"position_detial\" >"
         ...
         + "</div></a></li></ul><span class=\"bigCart\" onclick=\"Project.Load_Detail('" + number + "','" + point.lat + "','" + point.lng + "')\"></span></div></div>";
...
var Project = {
    ...
    Load_Result: function (resultlist) {
        ...
        $.each(resultlist, function (i, n) {
            ...
            htmlStr += "<li><a class=\"clearfix\" onclick=\"Project.Load_Detail('" + n.number + "','" + n.position + "')\">";
    ...
    Load_Detail: function (code, lat, lng) {
        PageUrlConfig.SetUrl();
        window.location.href = g_const_PageURL.ProjectDetail + "?code=" + code + "&lat=" + lat + "&lng=" + lng + "&city=" + encodeURIComponent(Project.api_input.city) + "&t=" + Math.random();
    },
