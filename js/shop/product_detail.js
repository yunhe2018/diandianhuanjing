$(document).ready(function () {
    ...
    $("#span_servermask_closed").click(function () {
        $("#div_servermask").hide();
    });
    $("#btn_telCS").click(function () {
        $("#span_tel").show();
        $("#span_QQ").hide();
        $("#div_servermask").show();
    });
    $("#btn_QQCS").click(function () {
        $("#span_QQ").show();
        $("#span_tel").hide();
        $("#div_servermask").show();
    });
...
var Product = {
    ...
    GetList: function () {
        ...
        request.done(function (msg) {
            if (msg.resultCode >= g_const_Success_Code) {
                ...
                $("#span_tel").html(msg.telCS);
                $("#span_QQ").html(msg.QQCS);
                // $("#discriptPic").html("<img src=\"" + msg.discriptPicList[0] + "\">");
                stockNum = msg.stockNum;
                var htmlStr = "";
                $.each(msg.discriptPicList, function (i, n) {
                    htmlStr += "<img src=\"" + n + "\" onerror=\"this.src='/img/default/pic_default_big.png'\">";
                });
                ...
                $("#orderCreate").click(function () {
                    ...
                    //将商品信息存入缓存中
                    localStorage[g_const_localStorage.ProductDetail] = JSON.stringify([{ "productCode": msg.productCode, "buyNum": parseInt($("#nums").val()), "currentPrice": msg.currentPrice }]);
    ...
    Load_Result: function (mainpicUrlA) {
        ...
        $.each(mainpicUrlA, function (i, n) {
            htmlStr += "<div class=\"swiper-slide\"><img src=\"" + n + "\" onerror=\"this.src='/img/default/pic_default_big.png'\"></div>";
        });
