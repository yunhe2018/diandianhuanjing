    ...
    GetList: function () {
        ...
        request.done(function (msg) {
            if (msg.resultCode >= g_const_Success_Code) {
                ...
                if (msg.isFollow == 1) {
                    Collection.Status = 1;
                    $("#b_AddCol").attr("class", "likeCon act");
                }
                else {
                    $("#b_AddCol").attr("class", "likeCon");
                }
                Project.ReportCount = msg.levelList.length;
                //Project.Load_Level(msg.levelList);

                if (msg.levelList[0].path.length>0) {
                    $("#btnBuyReport").html("查看报告");
                    $("#btnBuyReport").click(function () {
                        Project.Buy_Report();
                    });
                }
                else {
                    $("#btnBuyReport").html("￥" + msg.levelList[0].price + " 购买详细报告");
                    $("#btnBuyReport").click(function () {
                        Project.Buy_Report();
                    });
                }
...
var CommentAll = {
    ...
    Load_List: function (resultlist) {
        ...
        $.each(resultlist, function (i, n) {
            ...
            htmlStr += "<h2>" + n.phone.substr(0, 3) + "****" + n.phone.substr(7, 4) + "<strong>" + (n.orderTime != null ? n.orderTime.split(' ')[0] : "") + "</strong></h2>";
            htmlStr += "<div class=\"xingXingWrap\">";
            switch (n.level) {
                case 0:
                    htmlStr += "<img src=\"/img/xingXing.png\"><img src=\"/img/xingXing.png\"><img src=\"/img/xingXing.png\">";
                    break;
                case 1:
                    htmlStr += "<img src=\"/img/xingXing.png\"><img src=\"/img/xingXing.png\">";
                    break;
                case 2:
                    htmlStr += "<img src=\"/img/xingXing.png\">";
                    break;
            }
            htmlStr += "</div>";
